...
# nodeToNodeMeshEnabled to true or false
...
kubectl create clusterrolebinding calico-kube-controllers \ 
--clusterrole=cluster-admin --serviceaccount=kube-system:calico-kube-controllers --namespace=kube-system
```
...
data:
  ...
  etcd_endpoints: "https://172.26.6.1:4001,https://172.26.6.2:4001,https://172.26.6.3:4001"
...
typha_service_name "none"改为"calico-typha"。
  
- 禁用 node to node bgp mesh

```
export DATASTORE_TYPE=kubernetes KUBECONFIG=/opt/config
calicoctl get bgpconfig -o yaml > bgp.yaml
nodeToNodeMeshEnabled to true or false
calicoctl replace -f bgp.yaml
```
...
[配置global bgp peer](https://docs.projectcalico.org/v3.0/usage/routereflector/bird-rr-config)

```
apiVersion: projectcalico.org/v3
kind: BGPPeer
metadata:
  name: bgppeer-global-61
spec:
  peerIP: 172.26.6.1
  asNumber: 64512
```
